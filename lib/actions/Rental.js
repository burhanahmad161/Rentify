...
export const addRentalItem = async ({ title,image, description, location, category, price, priceUnit,owner }) => {
    try {
        ...
        const newRental = {
            ...
            isRented: false,
            rentedBy: "None",
            ratings: [],             // List of individual ratings { userId, stars }
            averageRating: 0  
        };
...
export const getAuctions = async () => {
  ...
  export const addRatingToItem = async ({ rentalId, userId, rating }) => {
    try {
      await client.connect(); // Connect to the database
      const db = client.db(dbName);
      const collection = db.collection("AuctionItems");
  
      console.log(rentalId,userId,rating);
      const objectId = new ObjectId(rentalId);
      ...
      const alreadyRated = item.ratings?.some(r => r.userId === userId);
      if (alreadyRated) {
        throw new Error("You have already rated this item");
      }
  
      // Add the new rating
      const updatedRatings = [...(item.ratings || []), { userId, rating }];
